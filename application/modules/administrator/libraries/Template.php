...
class Template
{
	...
	public function __construct()
	{
        ...
        $this->ci->lang->load('administrator/template', $this->ci->config->item('language'));
	}
	...
	public function view($content, $data = NULL)
	{
        ...
        else
        {
            ...
            $this->template['control_sidebar'] = $this->ci->load->view('_template/control_sidebar', $data, TRUE);
    ...
    public function alert($message, $config)
    {
        ...
        $alert .= "<small><strong><i class='ace-icon fa fa-{$config['icon']}'></i> {$config['title']}</strong>{$message}</small>";
        $alert .= "</div>";
