...
<html>
 <head>
  ...
  <style>
     ...
     table.gridtable td { border-width: 0.1px; vertical-align:  padding: 3px 3px 5px 3px; border-style: solid; border-color: black; }
     table.kop tr { line-height: 50%;  }
      table.kop > td > tr { font-size: 40px;  }
...
<?php endif; ?>
   <div class="content">
    <h3 class="upper text-center">Data Pengajuan Barang</h3>
    <table class="gridtable mini-font" width="100%">
      <thead>
        <tr class="mini-font">
          ...
          <th rowspan="2">Divisi Kerja</th>
          <th colspan="8">Barang yang diajukan</th>
        </tr>
        <tr>
          <th>Nama Barang</th>
          <th>jenis Barang</th>
...
foreach($pengajuan as $row) :
   $items_data = $this->pengajuan->get_items($row->ID_pengajuan);
?>
        <tr style="margin-top: -20px;">
          <td rowspan="<?php echo (count($items_data) + 1);  ?>"><?php echo $no++; ?>.</td>
          <td rowspan="<?php echo (count($items_data) + 1);  ?>"><?php echo str_replace('-', '/', $row->date) ?></td>
          <td rowspan="<?php echo (count($items_data) + 1);  ?>"><?php echo $row->full_name; ?></td>
          <td rowspan="<?php echo (count($items_data) + 1);  ?>"><?php echo $row->division_name; ?></td>
        </tr>
   <?php  
   /* Start Loops Itms */
   $total = 0;
...
?>
      ...
      <thead>
         <tr>
            <td colspan="10" style="text-align: right;">Total (Rp.) : </td>
